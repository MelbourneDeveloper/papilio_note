...
void main() {
  test('Test persistence', () async {
    ...
    expect(
      actualPersistedModel.settings.isDarkMode,
      expectedPersistedModel.settings.isDarkMode,
    );
    expect(
      actualPersistedModel.notes[0].id,
      expectedPersistedModel.notes[0].id,
    );
    expect(
      actualPersistedModel.notes.length,
      expectedPersistedModel.notes.length,
    );
  });
